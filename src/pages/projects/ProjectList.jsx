...
const ExpandedProjectDetails = ({ projectId }) => {
  ...
  if (isLoading) return <div className="p-6 text-center text-gray-500 text-sm">Memuat detail aplikasi...</div>;
  ...
  return (
    <div className="bg-gray-50 p-6 border-b border-gray-100">
      ...
      {project.timelines?.length === 0 ? (
        <div className="text-xs text-gray-500 italic">Belum ada timeline di aplikasi ini.</div>
      ) : (
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-full">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Daftar Aplikasi</h1>
        </div>
        <button
      ...
      <Modal
        isOpen={isModalOpen}
        onClose={closeModal}
        title={editingProject ? "Edit Project" : "Create Aplikasi Baru"}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-1">Judul Aplikasi</label>
            <input
              type="text"
              required
